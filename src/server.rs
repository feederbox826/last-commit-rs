...
static CORS_HEADER: LazyLock<Header> =
    LazyLock::new(|| Header::from_bytes("Access-Control-Allow-Origin", "*").unwrap());
...
fn handle(state: &AppState, req: tiny_http::Request) {
    ...
    match path {
        ...
        "/health" | "/favicon.ico" => {
            req.respond(Response::new_empty(StatusCode(204))).ok();
        }
        _ => {
            let name = &path[1..];
            ...
            if !refresh {
                ...
                if let Some(entry) = cache.get(name) {
                    if !cache::is_expired(entry) {
                        ...
                        let cache_header =
                            Header::from_bytes("Cache-Control", "public, max-age=604800").unwrap();
